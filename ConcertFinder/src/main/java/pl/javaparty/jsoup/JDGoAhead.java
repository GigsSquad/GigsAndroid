...
public class JDGoAhead {
	...
	/*
	 * Baze nalezy aktualizowac jeseli: - jeszcze nie mamy danych z tej strony - dane tej strony se przestarzale (dodano
	 * nowe koncerty) - sprawdzamy to por?wnuj?c hashcody najnowszych koncert?w
	 */
	private boolean needsToBeUpdated(int currentHash) {
		boolean check = false;
		if (!dbm.agencyHashCodeExists("GOAHEAD"))
			check = true;
		else if (dbm.getHash("GOAHEAD") != currentHash)
			check = true;
		if (!check)
	...
	/*
	 * public void getMoreData(Concert concert) throws IOException//GoAhead { Document doc =
	 * Jsoup.connect(concert.getURL()).get(); Elements concertData = doc.getElementsByClass("kk2"); int index = 0;
	 * String name = concertData.get(index++).text();//nie wiem co z tym zrobic, zostawiac, czy nie? String date =
	 * concertData.get(index++).text(); String city = concertData.get(index++).text(); String place =
	 * concertData.get(index++).text(); String adress = concertData.get(index++).text(); String entry =
	 * concertData.get(index++).text(); String ticketsPrice = concertData.get(index++).text(); String whereToBuy =
	 * concertData.get(index++).text(); concert.setMoreData(adress, entry, ticketsPrice); //return
	 * String.format("%s%n%s%n%s %s %s%n%s%n%s%n%s", name, date, city, place, adress,entry,ticketsPrice,whereToBuy); }
	 */
